...
$lessons = @{
    ...
    "lesson3" = @{ name = "第3课 植物妈妈有办法"; words = @(
    ...
    "lesson9" = @{ name = "第9课 黄山奇石"; words = @(
        @{ text = "南部"; pinyin = "nán bù" },
        @{ text = "那些"; pinyin = "nà xiē" },
        @{ text = "山顶"; pinyin = "shān dǐng" },
        @{ text = "一动不动"; pinyin = "yí dòng bú dòng" },
        @{ text = "云海"; pinyin = "yún hǎi" },
        @{ text = "前方"; pinyin = "qián fāng" },
        @{ text = "每当"; pinyin = "měi dāng" },
        @{ text = "金光闪闪"; pinyin = "jīn guāng shǎn shǎn" },
        @{ text = "它们"; pinyin = "tā men" }
    )}
}

Write-Host "开始导入语文二年级上册课文..." -ForegroundColor Green
...
foreach ($key in $lessons.Keys) {
    ...
    try {
        $response = Invoke-RestMethod -Uri "$baseUrl/api/builtin/chinese/grade2_semester1/$key" -Method PUT -Body $body -ContentType "application/json"
        Write-Host "✓ $($lesson.name)" -ForegroundColor Green
    } catch {
        Write-Host "✗ $($lesson.name): $($_.Exception.Message)" -ForegroundColor Red
    }
}

Write-Host ""
Write-Host "导入完成！共 $($lessons.Count) 课" -ForegroundColor Cyan
Write-Host "刷新页面即可看到课文列表" -ForegroundColor Cyan
